 ...
 *
 * Intercepts system notifications and transforms them into the shared contract format for
 * forwarding to the watchOS companion app.
 *
 * This service requires the BIND_NOTIFICATION_LISTENER_SERVICE permission and must be explicitly
 * granted access by the user through system settings.
 */
@AndroidEntryPoint
class PushedNotificationListener : NotificationListenerService() {
    ...
    @Inject lateinit var watchSyncManager: WatchSyncManager
    ...
    override fun onListenerDisconnected() {
        ...
        serviceScope.launch { notificationRepository.setListenerConnected(false) }
    }
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification) {
        ...
        serviceScope.launch {
            try {
                ...
                Log.d(
                        TAG,
                        "Processed notification: ${pushedNotification.title} from ${pushedNotification.appName}"
                )
            } catch (e: Exception) {
    ...
    /**
     * Process all currently active notifications. Called on listener connection to sync any
     * notifications that arrived while the listener was disconnected.
     */
    ...
    private fun transformNotification(sbn: StatusBarNotification): PushedNotification {
        ...
        return PushedNotification(
                ...
                color =
                        notification.color.takeIf { it != 0 }?.let {
    ...
    /**
     * Generate a stable, unique ID for the notification. Uses package name, tag, and ID to create
     * deterministic UUID.
     */
    ...
    private fun extractActions(notification: Notification): List<NotificationAction> {
        return notification.actions?.take(MAX_ACTIONS)?.map { action ->
            NotificationAction(
                    ...
                    requiresUnlock =
                            action.extras.getBoolean(Notification.EXTRA_SHOW_WHEN, false).not(),
                    icon = null
            )
        }
                ?: emptyList()
